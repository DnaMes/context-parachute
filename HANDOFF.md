...
(v1.0.1, done) and a feature (v1.1.0) designed via brainstorming.
...
- **v1.0.1 implemented, not yet tagged/pushed** — both bugfixes from the spec:
...
1. **Finish v1.0.1 shipping:** commit the 5 staged files
   (`VERSION CHANGELOG.md README.md hooks/parachute-watch.sh tests/run-tests.sh`),
   tag `v1.0.1` locally (consistency test requires tag == VERSION), then **ask
   before pushing** — push is shared state, not implied by "continue."
