...
if mkdir -p "$OUTPUT_DIR" 2>/dev/null; then
    EMERGENCY="${OUTPUT_DIR}/emergency.md"
    {
        ...
        printf '_Auto-written on auto-compaction at %s (the watcher never fired). Generated by context-parachute v%s._\n\n' "$TIMESTAMP" "$VERSION"
...
cat <<EOF
=== CONTEXT-PARACHUTE EMERGENCY BRAIN DUMP (v${VERSION}) — ${PROJECT} — ${TIMESTAMP} ===
